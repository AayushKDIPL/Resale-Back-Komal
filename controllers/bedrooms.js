...
const BedroomsController = {
  getProducts: async (req, res) => {
    try {
      ...
      const products = await bedrooms.find({})
        .sort({ priority: -1 })
        .skip(skipUsers)
        .limit(req.query.limit);

      const totalProducts = await bedrooms.find({}).count();
  ...
  getProduct: async (req, res) => {
    try {
      const data = await bedrooms.findById(req.params._id);
  ...
  addProduct: async (req, res) => {
    try {
        ...
        console.log('Incoming data:', bed);

        // Validate and create products
        const products = await bedrooms.create({
            bedrooms: bed
        });
  ...
  deleteProduct: async (req, res) => {
    try {
      let product = await bedrooms.findById(req.params._id);
      if (!product) throw new Error("Invalid call");
      await bedrooms.findByIdAndDelete(req.params._id);
