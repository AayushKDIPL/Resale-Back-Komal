...
const furnishingController = {
  getProducts: async (req, res) => {
    try {
      ...
      const products = await furnishing.find({})
        .sort({ priority: -1 })
        .skip(skipUsers)
        .limit(req.query.limit);

      const totalProducts = await furnishing.find({}).count();
  ...
  getProduct: async (req, res) => {
    try {
      const data = await furnishing.findById(req.params._id);
  ...
  addProduct: async (req, res) => {
    try {
        ...
        const products = await furnishing.create({
  ...
  deleteProduct: async (req, res) => {
    try {
      let product = await furnishing.findById(req.params._id);
      if (!product) throw new Error("Invalid call");
      await furnishing.findByIdAndDelete(req.params._id);
