...
const BudgetController = {
  getProducts: async (req, res) => {
    try {
      ...
      const products = await budget.find({})
        .sort({ priority: -1 })
        .skip(skipUsers)
        .limit(req.query.limit);

      const totalProducts = await budget.find({}).count();
  ...
  getProduct: async (req, res) => {
    try {
      const data = await budget.findById(req.params._id);
  ...
  addProduct: async (req, res) => {
    try {
        ...
        const products = await budget.create({budget: budgets});
  ...
  deleteProduct: async (req, res) => {
    try {
      let product = await budget.findById(req.params._id);
      if (!product) throw new Error("Invalid call");
      await budget.findByIdAndDelete(req.params._id);
