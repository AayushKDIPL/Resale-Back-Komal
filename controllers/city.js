...
const CityController = {
  getProducts: async (req, res) => {
    try {
      ...
      const products = await city.find({})
        .sort({ priority: -1 })
        .skip(skipUsers)
        .limit(req.query.limit);

      const totalProducts = await city.find({}).count();
  ...
  getProduct: async (req, res) => {
    try {
      const data = await city.findById(req.params._id);
  ...
  addProduct: async (req, res) => {
    try {
        ...
        const products = await city.create({
            city: cityy
        });
  ...
  deleteProduct: async (req, res) => {
    try {
      let product = await city.findById(req.params._id);
