...
const constructionController = {
  getProducts: async (req, res) => {
    try {
      ...
      const products = await construction.find({})
        .sort({ priority: -1 })
        .skip(skipUsers)
        .limit(req.query.limit);

      const totalProducts = await construction.find({}).count();
  ...
  getProduct: async (req, res) => {
    try {
      const data = await construction.findById(req.params._id);
  ...
  addProduct: async (req, res) => {
    try {
        ...
        const products = await construction.create({
  ...
  deleteProduct: async (req, res) => {
    try {
      let product = await construction.findById(req.params._id);
      if (!product) throw new Error("Invalid call");
      await construction.findByIdAndDelete(req.params._id);
