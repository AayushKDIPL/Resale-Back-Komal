...
const SaleRentController = {
  getProducts: async (req, res) => {
    try {
      ...
      const products = await saleRent.find({})
        .sort({ priority: -1 })
        .skip(skipUsers)
        .limit(req.query.limit);

      const totalProducts = await saleRent.find({}).count();
  ...
  getProduct: async (req, res) => {
    try {
      const data = await saleRent.findById(req.params._id);
  ...
  addProduct: async (req, res) => {
    try {
        ...
        const products = await saleRent.create({
            sale_rent: sale_rent
        });
  ...
  deleteProduct: async (req, res) => {
    try {
      let product = await saleRent.findById(req.params._id);
      if (!product) throw new Error("Invalid call");
      await saleRent.findByIdAndDelete(req.params._id);
