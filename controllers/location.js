...
const LocationController = {
  getProducts: async (req, res) => {
    try {
      ...
      const products = await location.find({})
        .sort({ priority: -1 })
        .skip(skipUsers)
        .populate('city')
        .limit(req.query.limit);

      const totalProducts = await location.find({}).count();
  ...
  getProduct: async (req, res) => {
    try {
      const data = await location.findById(req.params._id);
  ...
  addProduct: async (req, res) => {
    try {
        ...
        const products = await location.create({
            location: locationy,
            city: id
        });
  ...
  deleteProduct: async (req, res) => {
    try {
      let product = await location.findById(req.params._id);
